...
export const getSyncResponse = (payload: any) => {
    if (payload.method == "eth_accounts" && (typeof window.wootzappAppcurrentAccountAddress !== "undefined")) {
        return web3Response(payload, [ window.wootzappAppcurrentAccountAddress ])
    } else if (payload.method == "eth_coinbase" && (typeof window.wootzappAppcurrentAccountAddress !== "undefined")) {
        return web3Response(payload, window.wootzappAppcurrentAccountAddress)
    } else if (payload.method == "net_version" || payload.method == "eth_chainId") {
        return web3Response(payload, window.wootzappAppNetworkId)
